...
from .unet_2d_blocks import UNetMidBlock2D, get_down_block, get_up_block, Conv3DAwareUNetMidBlock2D
...
class Conv3DAwareUNet(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # input
        # self.conv_in = nn.Conv2d(in_channels, block_out_channels[0], kernel_size=3, padding=(1, 1))
        # self.conv_in = Conv3DAware(in_channels,block_out_channels[0],kernel_size=3,padding=1)
        self.conv_in = GroupConv(in_channels, block_out_channels[0], kernel_size=3, padding=1)
        # time
        if time_embedding_type == "fourier":
            self.time_proj = GaussianFourierProjection(embedding_size=block_out_channels[0], scale=16)
            timestep_input_dim = 2 * block_out_channels[0]
        ...
        self.conv_act = nn.SiLU()
        # self.conv_out = nn.Conv2d(block_out_channels[0], out_channels, kernel_size=3, padding=1)
        # self.conv_out = Conv3DAware(block_out_channels[0], out_channels, kernel_size=3, padding=1)
        self.conv_out = GroupConv(block_out_channels[0], out_channels, kernel_size=3, padding=1)
    def forward(
        self,
        sample: torch.FloatTensor,
        timestep: Union[torch.Tensor, float, int],
