...
from .resnet import Downsample2D,Downsample2D3DAware, FirDownsample2D, FirUpsample2D, KDownsample2D, KUpsample2D, ResnetBlock2D, Upsample2D , Upsample2D3DAware, Conv3dAwareResnetBlock2D
...
def get_down_block(
    ...
    elif down_block_type == "Conv3DAwareDownBlock2D":
...
def get_up_block(
    ...
    elif up_block_type == "Conv3DAwareUpBlock2D":
...
class Conv3DAwareUpBlock2D(nn.Module):
    def __init__(
        ...
        if add_upsample:
            # self.upsamplers = nn.ModuleList([Upsample2D(out_channels, use_conv=True, out_channels=out_channels)])
            self.upsamplers = nn.ModuleList([Upsample2D3DAware(out_channels, use_conv=True, out_channels=out_channels)])
        else:
            self.upsamplers = None
